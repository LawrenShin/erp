        ...
        return (
            this.props.uiLocation === 'header' ?
                <div className="header-profile">
                    <div role="listbox" aria-expanded="true" className="ui dropdown">
                        <div className="header-profile__content" onClick={() => this.setState(({show}) => ({show: !show}))}>
            ...
            :
                    ...
                    <div className="user-profile">
                        <div style={{cursor: "pointer"}} className="user-profile__initials" onClick={() => history.push("/user")}>{user.first_name[0]}{user.last_name[0]}</div>
                        <div className="user-profile__title">{user.first_name} {user.last_name}</div>
                        <div className="user-profile__post" style={{fontSize: user.email.length < 20 ? '1em' : '0.9em'}}>{user.email}</div>
                        {user.role && <div className="user-profile__post">{user.role[0].toUpperCase()}{user.role.substr(1).toLowerCase()}</div>}
                    </div>
