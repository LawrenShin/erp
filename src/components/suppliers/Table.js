...
class DynamicTable extends React.PureComponent {
    ...
    fields = {
        ...
        get_reliability_rating: "Reliability rating",
    ...
    async onChange(props, prevProps) {
        ...
        this.setState({list: this.props.list.data, loading: this.props.list.state === 'loading' ? true : false})
        if (!isEqual) {
    ...
    onScroll = async ({valueY, valueX, height}) => {
        ...
        if (valueY / height > 0.9 && valueY > this.backY && this.offset + this.state.options.limit < this.count) {
            ...
            this.setState({loading: true}, async () => {
                ...
                this.setState(async ({list}) => ({
                    list: await getApi("supplier").run("list", this.state.options).results.concat(data),
                    loading: false
                }));
            });
        }
    ...
    render() {
        ...
        const categories = this.props.store.suppliers.filters.data.categories
        ...
        return (
            <div style={{position: "relative"}}>
                <Table
                    ...
                    cell={(value, name, row) =>
                        name === "q" ?
                            <span className="action-quatation action-quatation_add">Add supplier to quotation</span> :
                            name === "id" ? (
                                    this.props.addToQuotation ?
                                        ...
                                        <>
                                            <div className="-flex">
                                                <Expand active={row === this.state.rowIndex} onClick={() => {
                                ...
                                name === "name" ? <>
                                        <div className="col-name">{this.state.list[row].name.replace("&nbsp;", " ")}</div>
                                        <span>{this.state.list[row].type === 'FC' ? 'Factory' : 'Agent'}</span></> :
                                    name === "status" ?
                                        (<>
                                            {this.state.list[row].status === 'IN' &&
                                            <div className="col-status col-status_inactive">stop</div>}
                                            {this.state.list[row].status === 'AC' &&
                                            <div className="col-status col-status_active">active</div>}
                                            {this.state.list[row].status === 'TS' &&
                                            <div className="col-status col-status_test">test</div>}
                                        </>) :
                                        name === "factory_country" ?
                                            (
                                                <>
                                                    {this.state.list[row].factory_country && this.state.countries && this.state.countries.find(({id}) => this.state.list[row].factory_country === id) ?
                                                        <ReactCountryFlag
                                                            code={this.state.countries.find(({id}) => this.state.list[row].factory_country === id).iso.toLowerCase()}
                                                            svg styleProps={{width: '20px', height: '10px'}}/> : null}
                                                    {this.state.list[row].factory_country && this.state.countries && this.state.countries.find(({id}) => this.state.list[row].factory_country === id) ?
                                                        <span>{this.state.countries.find(({id}) => this.state.list[row].factory_country === id).name}</span> : null}
                                                </>
                                            ) :
                                            name === "legal_country" ?
                                                (
                                                    <>
                                                        {this.state.list[row].legal_country && this.state.countries && this.state.countries.find(({id}) => this.state.list[row].legal_country === id) ?
                                                            <ReactCountryFlag
                                                                code={this.state.countries.find(({id}) => this.state.list[row].legal_country === id).iso.toLowerCase()}
                                                                svg
                                                                styleProps={{width: '20px', height: '10px'}}/> : null}
                                                        {this.state.list[row].legal_country && this.state.countries && this.state.countries.find(({id}) => this.state.list[row].legal_country === id) ?
                                                            <span>{this.state.countries.find(({id}) => this.state.list[row].legal_country === id).name}</span> : null}
                                                    </>
                                                ) :
                                                (name === "categories" && "categories" in this.state.list[row] && this.state.list[row].categories.length) ?
                                                    (
                                                        (this.state.list[row].categories.length && categories.length) ?
                                                            <span>
                      {
                          this.state.list[row].categories
                              .map(v => {
                                  return categories.filter((category) => category.value === v).map(c => c.text)
                              }).join(', ')
                      }
                      </span> : ''
                                                    ) :
                                                    name === "payment_terms_id" ? (() => {
                                                            // IIFE for matching paymentTerms ids with paymentTerms values
                                                            const match = this.props.store.suppliers.paymentTerms.data.filter(pt => pt.value === this.state.list[row].payment_terms)[0]
