...
import { generalFormSubmit, contactFormSubmit, optionsFormSubmit, bankFormSubmit, stepBack } from '../../../actions/createSupplier.js';
...
class CreateSupplierPage extends Component{
  state = { generalInfo: {}, errors: false, modified: false };
  ...
  createSupplier = (bankInfo) => {
    ...
    promise.then((res) => {
      ...
      if((categories.length || ages.length || genders.length) && supplierId){
        ...
        promiseSaveCategories.then((res) => { results.push(res.length); }).catch((err) => { console.log(err); });
        promiseSaveGenders.then((res) => { results.push(res.length); }).catch((err) => { console.log(err); });
        promiseSaveAges.then((res) => { results.push(res.length); }).catch((err) => { console.log(err); });
      }
      ...
      if(incoterms.length && supplierId){
        ...
        promiseSaveContacts.then((res) => { results.push(res.length); })
      }
  ...
  next = () => {
    this.props.dispatch(generalFormSubmit({}));
  }
  ...
  render(){
    ...
    return(
      <>
        ...
        {step === 0 ? <GeneralForm info={this.props.createSupplier.general_info} promiseTerms={promiseTerms} saveStepInfo={this.saveStepInfo} /> : null}
        {step === 1 ? <ContactForm info={this.props.createSupplier.contact_info} saveStepInfo={this.saveStepInfo} stepBack={this.stepBack} /> : null}
        {step === 2 ? <OptionsForm options_info={this.props.createSupplier.options_info} saveStepInfo={this.saveStepInfo} stepBack={this.stepBack} /> : null}
        {step === 3 ? <BankForm info={this.props.createSupplier.bank_info} createSupplier={this.createSupplier} saveStepInfo={this.saveStepInfo} stepBack={this.stepBack} /> : null}
        {Object.keys(this.state.errors).length > 0 && <div>
          {Object.keys(this.state.errors).map((error) => <span>{`${error} - ${this.state.errors[error]}`}</span>)}
        </div>}
      </>
    );
  }
