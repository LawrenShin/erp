...
class ContactForm extends Component{
  ...
  renderFields = (field, index, incoterm) => {
    ...
    return (
      <Input key={index} onChange={this.localSave} label={field.name} name={field.inputName} value={ this.state.info[field.inputName] ? this.state.info[field.inputName] : '' } />
    )
  ...
  localSave = ({name, value, checked}) => {
    ...
    switch(name){
      case 'legal_city': this.setState({ legalCity: this.state.cities.filter((city) => city.id === value)[0] }, () => this.proceed(0));break;
      case 'legal_city_factory': this.setState({ legalCityFactory: this.state.cities.filter((city) => city.id === value)[0] }, () => this.proceed(0));break;
      case 'legal_country': this.setState({ legalCountry: this.state.countries.filter((country) => country.id === value)[0] }, () => this.proceed(0));break;
      case 'legal_country_factory': this.setState({ legalCountryFactory: this.state.countries.filter((country) => country.id === value)[0] }, () => this.proceed(0));break;
  ...
  getIncotermsFormRef = (node) => { this._incotermsForm = node };
      ...
      return(
        <div className="bg-box box-supplier">
            ...
            <div className="form-box">
                ...
                <CheckboxComponent className="form-box__item checkbox-card" onChange={this.localSave} label=' factory address is the same' name='factory_address_same' />
            </div>
          </form>
