        ...
        {this.state.fields.map((field, index) => (
          <div className="table-add__td">
            ...
            <div className="select-elem">
               <Dropdown name={field} key={field} placeholder="CIF" selection options={this.state.cifOptions} onChange={this.props.localSave} />
             </div>
          </div>        
        ))}
        <div className="table-add__td td-remove">
          <span style={{cursor: "pointer"}} className="table-add__remove"><i className="icon-trash"></i></span>
        </div>
      </div>
   );
  };
            ...
            <div className="delivery-terms">
              <div className="table-add">
                <div className="table-add__row table-add__head">
                  {this.state.fields.map((field, index) => {if(field.indexOf('icon_') < 0) return <div className="table-add__th" key={field}>{field}</div>})}
